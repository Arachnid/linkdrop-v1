...
class ErrorPage extends React.Component {
  render () {
    ...
    const instructions = error === 'CONNECTOR_NETWORK_NOT_SUPPORTED' || error === 'NETWORK_NOT_SUPPORTED' || error === 'NEED_METAMASK' ? this.t(`errors.${error}.instructions`, { returnObjects: true, network }) : {}
    const description = this.t(`errors.${error}.description`, { network })
    return <div className={commonStyles.container}>
      <Alert className={styles.alert} icon={this.defineIcon({ error })} />
      <div className={styles.title}>{this.t(`errors.${error}.title`, { network })}</div>
