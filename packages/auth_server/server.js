...
app.use(cors({
//    origin: 'https://coinartist.tweetdrop.io'
}))
...
connectDB()
  .then(() => {
    // Run server
    const PORT = process.env.PORT || process.env.CUSTOM_PORT || 5001
    
    twitterHandleService.loadCache()
    linkdropService.loadCache()
...
// Define routes
app.get('/', (req, res) => res.send('👋  Hello from linkdrop auth server'))
